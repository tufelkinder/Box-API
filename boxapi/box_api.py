...
class BoxFile(JSONAwareObject):
    ...
    FIELDS = ['box_type','id','sequence_id','etag','sha1','name','description',
              ...
              'shared_link','parent','item_status','version_number','comment_count',
              'permissions','tags','lock']
    ...
    def setVersionNumber(self,version_number):
        self._version_number = version_number

    version_number = property(getVersionNumber,setVersionNumber,doc="")


    def getCommentCount(self):
        return self._comment_count

    def setCommentCount(self,comment_count):
        self._comment_count = comment_count

    comment_count = property(getCommentCount,setCommentCount,doc="")
    ...
    permissions = property(getPermissions,setPermissions,doc="")
    ...
    tags = property(getTags,setTags,doc="")
    ...
    lock = property(getLock,setLock,doc="")
    ...
    extension = property(getExtension,setExtension,doc="")
...
class BoxFolder(JSONAwareObject):
    ...
    FIELDS = ['box_type','id','sequence_id','etag','name','created_at','modified_at','description',
              ...
              'folder_upload_email','parent','item_status','item_collection','sync_state','has_collaborations',
              'permissions','tags']
    ...
    def getHasCollaborations(self):
        return self._has_collaborations

    def setHasCollaborations(self,has_collaborations):
        self._has_collaborations = has_collaborations

    has_collaborations = property(getHasCollaborations,setHasCollaborations,doc="")
...
#            print >> sys.stderr, 'No can do nothing without not a client_id and a client_secret, yo.'
    ...
    def getHeaders(self):
        if self.tokenExpired():
            ...
            if self.profile and refresh_result:
                self.profile.update_auth(self.getUserAuthInfo())
    ...
    # folder_dict is a dictionary or folder object of changed values
    def updateFolderInfo(self,folder_id,folder_dict):
        headers = self.getHeaders()
        url = self.base_url + '/folders/{0}'.format(folder_id)
        data = json.dumps(folder_dict)
        response = requests.put(url,headers=headers,data=data)
        folder_info = response.json()
        bf = BoxFolder(box_type='folder')
        bf.newFromJsonDict(bf,folder_info)
    ...
    def updateFileInfo(self, file_id, fields_dict):
        headers = self.getHeaders()
        url = self.base_url + '/files/{0}'.format(file_id)
        response = requests.put(url,headers=headers,data=json.dumps(fields_dict))
        file_info = response.json()
        bf = BoxFile(box_type='file')
        bf.newFromJsonDict(bf,file_info)
        return bf
    ...
    def getFileComments(self,file_id):
        headers = self.getHeaders()
        url = self.base_url + '/files/{0}/comments'.format(file_id)
        response = requests.get(url,headers=headers)
        comment_info = response.json()
        results = []
        try:
            entries = comment_info['entries']
        except:
            return comment_info
        for entry in entries:
            com = Comment(box_type='comment')
            com.newFromJsonDict(com,entry)
            results.append(com)
        return results
    ...
    def getCommentInfo(self,comment_id):
        headers = self.getHeaders()
        url = self.base_url + '/comment/{0}/'.format(comment_id)
        response = requests.get(url,headers=headers)
        comment_info = response.json()
        com = Comment(box_type='comment')
        com.newFromJsonDict(com,comment_info)
        return com
